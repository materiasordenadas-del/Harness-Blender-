...
def diff_reports(before: dict[str, Any], after: dict[str, Any]) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {}
    for key in ("vertices", "edges", "faces", "boundary_edges", "non_manifold_edges", "loose_edges", "degenerate_faces", "self_intersections", "inconsistent_normal_edges"):
    ...
    for key in ("max_radius_jump", "max_curvature_radians", "minimum_thickness", "maximum_thickness"):
        if key in before and key in after:
            result[f"{key}_delta_percent"] = _percent(before[key], after[key])
    return result
