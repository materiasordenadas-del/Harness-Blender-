...
| Rig | Auto-Rigging | 5 (includes walking + running) |
| **Total** | | **35** |
...
| Change mesh format/topology | Remesh | `POST /openapi/v1/remesh` |
| Convert a model to other formats | Convert | `POST /openapi/v1/convert` |
| Rescale a model | Resize | `POST /openapi/v1/resize` |
| Generate fresh UVs | UV Unwrap | `POST /openapi/v1/uv-unwrap` |
| Add skeleton to character | Auto-Rigging | `POST /openapi/v1/rigging` |
| Animate a rigged character | Animation | `POST /openapi/v1/animations` |
| Check credit balance | Balance | `GET /openapi/v1/balance` |
...
### Optional 2D Optimization Pre-Step

Prefer image-to-3D when a clean reference can be produced. Before image-to-3D, evaluate whether the source image needs background removal, higher resolution, normalized lighting, or multiple views.
...
5. Credits consumed and remaining balance.
6. Suggested next operations such as refine, rig, animate, remesh or export.

---

## Error Recovery

On any failure, follow [references/troubleshooting.md](references/troubleshooting.md). The bundled CLI handles common HTTP errors and task failures.

---

## Known Behaviors & Constraints

- Tasks may remain at 99% during finalization.
- API requests should run server-side.
- Generated assets have limited retention; download what must be kept.
- PBR maps require `enable_pbr: true`.
- Check `model_urls` before assuming a format is present.
- Do not hard-code animation action IDs.
- Rigging requires a textured humanoid mesh.
- Large refined GLBs can be substantial; stream downloads rather than loading them wholly into memory.
...
- [ ] Used the bundled workflow helpers instead of reconstructing API behavior from memory.
